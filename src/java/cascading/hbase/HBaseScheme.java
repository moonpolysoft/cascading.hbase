...
public class HBaseScheme extends Scheme {
  ...
  /**
   * Constructor HBaseScheme creates a new HBaseScheme instance using fully
  ...
  public String[] getFamilyNames() {
    ...
    if (isFullyQualified) {
      for (String columnName : columns(null, this.valueFields)) {
        ...
        familyNameSet.add(hbaseColumn(pos > 0 ? columnName.substring(0, pos)
            : columnName));
      }
    } else {
  ...
  public Tuple source(Object key, Object value) {
    ...
    for (int i = 0; i < familyNames.length; i++) {
      ...
      for (Comparable comparable : cfFields) {
        ...
        byte[] cell = row.getValue(familyNames[i].getBytes(), qualifier
            .getBytes());
        result.add(cell);
      }
    }
    ...
    for (int i = 0; i < valueFields.length; i++) {
      ...
      for (int j = 0; j < values.getFields().size(); j++) {
        ...
        if (isFullyQualified)
          ...
          batchUpdate.put(hbaseColumn(familyNames[i])
              + fields.get(j).toString(), Bytes.toBytes(tuple.getString(j)));
